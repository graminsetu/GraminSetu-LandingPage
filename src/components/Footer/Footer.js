...
function Toast({ message, type, onClose }) {
    ...
    return (
        <div className={`fixed top-8 left-1/2 transform -translate-x-1/2 z-50 px-6 py-3 rounded-lg shadow-lg text-white text-base font-semibold transition-all ${type === 'error' ? 'bg-red-600' : 'bg-gramin-700'}`}>
            {message}
            <button onClick={onClose} className="ml-4 text-white/80 hover:text-white text-lg font-bold focus:outline-none">&times;</button>
        </div>
    );
}
...
function Footer() {
    ...
    return (
        <footer id="contact" className="bg-gradient-to-br from-gramin-900 via-gramin-800 to-gramin-900 text-white pt-12 pb-8 relative overflow-hidden">
            <div className="absolute inset-0 pointer-events-none opacity-10" style={{background: 'radial-gradient(circle at 80% 10%, #00e6a0 0%, transparent 60%)'}}></div>
            <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 relative z-10">
                ...
                <div className="w-full grid grid-cols-1 md:grid-cols-[2fr_0.8fr_1fr] gap-y-8 gap-x-8 md:gap-x-16 items-start justify-center mb-10">
                    ...
                    <div className="flex flex-col items-center md:items-start w-full md:max-w-lg mb-8 md:mb-0 md:mr-10" style={{flexBasis: '0', flexGrow: 2}}>
                        <div id="newsletter" className="bg-white/90 shadow-2xl rounded-2xl px-8 py-10 w-full max-w-xl flex flex-col items-center border border-gramin-100 backdrop-blur-md mx-auto">
                            <h3 className="text-2xl font-extrabold mb-1 flex items-center gap-2 leading-tight text-gramin-800 text-center">Stay Tuned for Demos & Updates</h3>
                            <p className="text-gramin-600 mb-4 text-base leading-relaxed max-w-md text-center">Get notified about GraminSetu demos, beta launches, and updates directly in your inbox.</p>
                            <form className="flex flex-col sm:flex-row items-center gap-2 w-full justify-center" onSubmit={async (e) => {
                                ...
                                try {
                                    ...
                                    if (response.ok) {
                                        setToast({ message: 'Thank you! You are now subscribed for updates and demo invites.', type: 'success' });
                                        e.target.reset();
                                    } else {
                                        setToast({ message: 'Failed to subscribe. Please try again later.', type: 'error' });
                                    }
                                } catch (error) {
