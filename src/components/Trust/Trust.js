...
function MetricCard({ metric, visible, delay }) {
  ...
  return (
    <div className="bg-[#f6fcf8] rounded-2xl shadow-md flex flex-col items-center justify-center p-8 transition-transform duration-200 hover:scale-105 hover:shadow-xl group cursor-pointer border border-gramin-100 relative">
      <div className="mb-3">{metric.icon}</div>
      <div className="text-4xl md:text-5xl font-extrabold text-gramin-700 mb-1 flex items-end">
        {metric.label === 'Funds Raised' ? <span>₹</span> : null}
        {value.toLocaleString()}
      </div>
      <div className="text-lg text-gramin-700 font-semibold mb-1">{metric.label}</div>
      {/* Tooltip/info icon */}
      <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
        <span className="bg-gramin-700 text-white text-xs rounded px-2 py-1 shadow">
          {metric.label}
        </span>
      </div>
    </div>
    ...
    <section ref={sectionRef} className="py-24 bg-white w-full">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col items-center">
        ...
        <div className="flex flex-col items-center mb-12">
          <div className="text-2xl font-bold text-gramin-800 mb-3 flex items-center gap-2">
        ...
        <div className="text-gramin-600 text-base mb-4 text-center max-w-xl">
          Your data is always private and secure. We never share your information without your
          consent.
        </div>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <a href="/terms" className="text-gramin-600 hover:text-gramin-800 underline">
            Terms of Service
          </a>
          <a href="/privacy" className="text-gramin-600 hover:text-gramin-800 underline">
            Privacy Policy
          </a>
        </div>
      </div>
