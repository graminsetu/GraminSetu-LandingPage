...
const pricingPlans = {
  monthly: [
    ...
    },
    {
      ...
      summary:
        'All Advanced features + multi-region management, dedicated support, custom integrations',
...
const features = [
  {
    icon: '🛰️',
    title: 'Real-Time Village Monitoring',
    desc: 'Access real-time data on village activities, scheme implementations, infrastructure projects, and community well-being — enabling better decision making and performance tracking.',
  },
  {
    icon: '📑',
    title: 'Compliance & Transparency Tools',
    desc: 'Maintain digital records of fund allocations, audit trails, and beneficiary lists. Facilitate accountability in governance with fully transparent workflows accessible by stakeholders.',
  },
  {
    icon: '🛠️',
    title: 'Issue Tracking & Resolution Management',
    desc: 'Track civic complaints and their resolution status across villages, enabling faster responses and proactive governance.',
  },
  {
    icon: '📢',
    title: 'Direct Community Engagement',
    desc: 'Broadcast announcements, schedule government-led events, and collect citizen feedback, ensuring two-way communication and participatory governance.',
  },
  {
    icon: '🔗',
    title: 'Integration with Government Systems',
    desc: 'Leverage APIs to synchronize data seamlessly with existing government portals, databases, and analytics platforms for cohesive operations.',
  },
  {
    icon: '📊',
    title: 'Reporting & Analytics',
    desc: 'Generate comprehensive reports on scheme coverage, fund utilization, and project impact with customizable dashboards tailored for different administrative levels.',
  },
];
    ...
    <div
      ...
      style={{
        ...
        backgroundImage: highlight
          ? 'radial-gradient(circle at 60% 40%, #e0ffe6 0%, #fff 100%)'
          : undefined,
      }}
    >
      {highlight && (
      ...
      <div
        className={`transition-all duration-300 w-full ${open ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}
        style={{ marginBottom: open ? 16 : 0 }}
      >
        {open && (
          <ul
            id={`plan-details-${name}`}
            className="mb-4 text-gramin-700 text-base list-disc list-inside text-left w-full space-y-2 bg-gramin-50 rounded-xl p-4 border border-gramin-100 animate-fade-in"
          >
            {details.map((f, j) => (
    ...
    <div className="bg-gradient-to-br from-gramin-50 via-white to-setu-50 min-h-screen pb-12">
      ...
      <section className="max-w-5xl mx-auto px-4 py-14 text-center flex flex-col items-center">
        <h1 className="text-5xl md:text-6xl font-extrabold text-gramin-900 mb-4 leading-tight tracking-tight drop-shadow-lg bg-gradient-to-r from-gramin-600 via-gramin-400 to-setu-400 bg-clip-text text-transparent">
          Explore Our Government & CSR Model
        </h1>
        <p className="text-2xl text-gramin-700 mb-6 font-semibold">
          Promote Transparent Governance & Accelerate Rural Development
        </p>
        <p className="text-lg text-gramin-600 mb-8 max-w-3xl">
          GraminSetu’s Government and CSR portal empowers public agencies and socially responsible
          enterprises to efficiently monitor village-level programs, foster transparency, and engage
          with communities in real time. Drive impactful governance and development outcomes through
          technology tailored for rural realities.
        </p>
        <div className="w-full flex justify-center mb-6">
          <div className="rounded-3xl overflow-hidden shadow-2xl border-4 border-gramin-100 bg-white">
            <img
              src={require('../assets/Before-and-after-images/govt-b-a.png')}
              alt="GraminSetu: Transparent Governance & Development"
